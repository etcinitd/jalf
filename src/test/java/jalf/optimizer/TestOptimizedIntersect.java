...
public class TestOptimizedIntersect extends OptimizerTest{
    ...
    public void testProject(){
        Relation operator = intersect(left, right);
        Relation optimized = optimized(operator).project(attrs(NAME, CITY));

        Relation expected = intersect(
    ...
    public void testRestrict(){
        ...
        Relation expected = left
                .restrict(eq(CITY, "C7"))
                .intersect(right.restrict(eq(CITY, "C7")));
        assertSameExpression(expected, optimized);
    }
